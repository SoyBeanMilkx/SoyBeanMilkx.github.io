    ...
    function() {
        ...
        m.viewer = m.plain + m.viewer,
            r.tokenize = function(e) {
                var n = ["\\\\begin{(.*?)}([\\s\\S]*?)\\\\end{\\1}", "\\\\\\[[\\s\\S]*?\\\\\\]", "\\\\\\([\\s\\S]*?\\\\\\)", "\\\\\\$", "\\$\\$(?:[^\\\\]|\\\\.)*?\\$\\$", "\\$(?:[^$\\\\]|\\\\.)+?\\$", r.tokenLiteral.MASK].join("|");
                const t = new RegExp(n, "g");
                let o,
                    d;
                const i = [];
                let a = 0;
                for (; null !== (o = t.exec(e));) o.index > a && (d = e.substring(a, o.index), i.push([r.tokenType.MARK, d])),
                    void 0 !== o[1] && o[1].startsWith("md") ? i.push([r.tokenType.MARK, o[2]]) : i.push([r.tokenType.MASK, o[0]]),
                    a = t.lastIndex;
            ...
            r.renderPage = function() {
                ...
                let t;
                0 < e.length ? (t = e[0].value.trim(), e[0].remove()) : (t = window.document.body.innerHTML.trim(), window.document.body.innerHTML = ""),
                void 0 !== window.document.title && "" !== window.document.title || (d = t.split("\n", 1)[0].replace(/^\s*#*\s*|\s*#*\s*$/g, ""), window.document.title = d),
            ...
            r.main = function() {
                if (r.setDefaultOptions(), "undefined" != typeof window) {
                    for (const e in a)"undefined" != typeof window && void 0 !== window.texme && void 0 !== window.texme[e] && (a[e] = window.texme[e]);
                    t(a.markdownURL,
                        function() {
